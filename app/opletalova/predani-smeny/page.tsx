...
export default function PredaniSmenyPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('opletalova-predani-smeny')
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (guides.length > 0) {
      localStorage.setItem('opletalova-predani-smeny', JSON.stringify(guides))
    }
  }, [guides])
  ...
  return (
    <div className="container">
      <div className="header">
        <h1>Opletalova - Předání směny</h1>
        <nav>
          <Link href="/">Domů</Link>
        </nav>
      </div>
